...
class LinkPredModel(nn.Module):
    def __init__(self, input_dim, hidden_dim, n_layers=3, dropout_rate=0.5):
        """
        Args:
        ...
        self.extractor = SAGE(input_dim, hidden_dim, n_layers, dropout_rate)
    ...
    def forward(self, x, blocks, pos_graph, neg_graph):
        h = self.extractor(x, blocks)
        scores_pos, scores_neg = self.decode(h, pos_graph, neg_graph)
...
def train(model, score_func, train_pos, x, optimizer, batch_size):
    ...
    for perm in DataLoader(range(train_pos.size(0)), batch_size,
        ...
        g = dgl.graph((row, col), num_nodes=x.shape[0])

        # If the SparseTensor has values (edge weights), add them as edge features
        if value is not None:
            g.edata['weight'] = value

        # print(adj)

        # h = model(x, adj)
        h = model.get_node_emb(x, g)
...
def evaluate_mrr(device, data, model, score_func, split='test'):
    ...
    with torch.no_grad():
        ...
        row, col, value = adj.coo()  # row: source nodes, col: destination nodes
        # Create a DGL graph from the edges
        g = dgl.graph((row, col), num_nodes=data['x'].shape[0])
        # If the SparseTensor has values (edge weights), add them as edge features
        if value is not None:
            g.edata['weight'] = value
        node_emb = model.get_node_emb(data['x'].to(device), g.to(device))
...
def train_and_eval(param_grid):
    ...
    model = SAGE(384, 384, gnn_layers, dropout)
    if score_model == 'Asym':
